...
\item メール転送パラメータの設定。\\
エラー発生時のログもしくは出力レポートをメールで転送する機能を持つスクリプトがあります。
ログはgzip形式に圧縮された後に転送されます。
メール転送の各種パラメータはスクリプト実行時に設定ファイル名を{\tt -m MAILCONF}オプションで渡す形で指定します。
以下にvcd-trend.rb実行時に用いる設定ファイルの例を示します。
...
Usage: vcd-report.rb [cmd-options]
    ...
    -s, --skip                       Skip processing vSphere data
    -l, --logfile LOGFILEPATH        Log file name
    -t, --tempfile                   Output log to temporary file
    -h, --help                       Display this help
\end{verbatim}
...
\begin{Verbatim}[frame=single]
$ vcd-report.rb -v1 -l$VCDKIT/logs/vcd-ex.log \
  -t -m $VCDKIT/conf/mail/vcd-ex.xml
\end{Verbatim}

\begin{itemize}
\item フェーズ２本番環境のvCloud Directorに対してログイン操作を行います。
...
\begin{Verbatim}[frame=single]
$ vcd-report.rb -v1 -T -P -l$VCDKIT/logs/vcd-ex.log
\end{Verbatim}
