...
\begin{enumerate}
\item 配布パッケージ(vcdkit-1.0.1.tgz)をインストール先サーバのローカルディスクにコピー
...
0 1 */2 * * /opt/vmware/vcdkit/cron/vcd-trend.sh
...
\begin{verbatim}
$ vcd-dump-rb \
   -v vcd.vcdc.whitecloud.jp,System,vcdadminl \
   -c 10.128.0.57,vcdadmin \
...
vcd-report.rb で作成したレポートデータをもとにトレンド分析を行うための各種レポートを作成します。
...
\begin{itemize}
\item \$VCDKIT/data/vcd-report以下のすべてのディレクトリを検索し、検知されたレポートデータ区間内のトレンドレポートを作成します。

例：
\begin{quote}
\$VCDKIT/data/vcd-report 配下に 2011-07-01\_00-00-00 から 2011-07-31\_00-00-00までのレポートディレクトリがある場合、
\$VCDKIT/data/vcd-trend/2011-07-01\_00-00-00\_\_2011-07-31\_00-00-00配下にトレンドレポートを作成します。
\end{quote}
...
\textbf{ファイル名}　& \textbf{説明} \\ 
\midrule
WinGuest.XML & 各VDC毎/各ゲストOSタイプ毎のVM数の内訳（最少数、最大数、平均値）\\
\bottomrule
\end{tabular}
\end{table}
...
{\footnotesize 
\begin{verbatim}
1 | for d in vcd-dump vcd-report
2 | do
3 |   for t in `find $VCDDATA/$d -maxdepth 1 -mindepth 1 -mtime +7 -type d`
4 |   do
5 |     file=`basename $t`
6 |     dir=`dirname $t`
7 |     echo "Creating tar archive: $t"
8 |     tar zcf $dir/$file.tgz -C $dir $file && rm -fr $t
9 |   done
10|   mv $VCDDATA/$d/*.tgz $VCDDATA/$d/archive
11| done
\end{verbatim}
}
\$VCDDATA/vcd-dumpおよび\$VCDDATA/vcd-report直下の、７日前より古いすべてのディレクトリ
（3行目のfindコマンド）が、圧縮されたtarアーカイブに変換されます。何らかの理由でアーカイブ作成に
失敗した場合には元のディレクトリはそのまま残ります（8行目の\&\&で連結されたtarおよびrmコマンド）。
作成されたtarアーカイブはarchiveディレクトリに移動します（10行目のmvコマンド）。
\end{document}
